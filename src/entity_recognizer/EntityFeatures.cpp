...
void EntityFeatures::AddUnigramFeatures(SequenceInstanceNumeric *sentence,
  ...
  AddFeature(fkey, features);
  fkey = encoder_.CreateFKey_PP(EntityFeatureTemplateUnigram::PpP, flags, PID, pPID);
  AddFeature(fkey, features);
  fkey = encoder_.CreateFKey_PP(EntityFeatureTemplateUnigram::PnP, flags, PID, nPID);
  AddFeature(fkey, features);
  fkey = encoder_.CreateFKey_PPP(EntityFeatureTemplateUnigram::PpPppP, flags, PID, pPID, ppPID);
  AddFeature(fkey, features);
  fkey = encoder_.CreateFKey_PPP(EntityFeatureTemplateUnigram::PnPnnP, flags, PID, nPID, nnPID);
  AddFeature(fkey, features);

  // Shape features.
  fkey = encoder_.CreateFKey_W(EntityFeatureTemplateUnigram::S, flags, SID);
  AddFeature(fkey, features);
  fkey = encoder_.CreateFKey_W(EntityFeatureTemplateUnigram::pS, flags, pSID);
  AddFeature(fkey, features);
  fkey = encoder_.CreateFKey_W(EntityFeatureTemplateUnigram::nS, flags, nSID);
  AddFeature(fkey, features);
  fkey = encoder_.CreateFKey_W(EntityFeatureTemplateUnigram::ppS, flags, ppSID);
  AddFeature(fkey, features);
  fkey = encoder_.CreateFKey_W(EntityFeatureTemplateUnigram::nnS, flags, nnSID);
  AddFeature(fkey, features);

  // Prefix/Suffix features.
  for (int l = 0; l < AID.size(); ++l) {
    uint8_t flag_prefix_length = l;
    fkey = encoder_.CreateFKey_WP(EntityFeatureTemplateUnigram::A, flags, AID[l], flag_prefix_length);
    AddFeature(fkey, features);
  }
  for (int l = 0; l < ZID.size(); ++l) {
    uint8_t flag_suffix_length = l;
    fkey = encoder_.CreateFKey_WP(EntityFeatureTemplateUnigram::Z, flags, ZID[l], flag_suffix_length);
    AddFeature(fkey, features);
  }
...
void EntityFeatures::AddBigramFeatures(SequenceInstanceNumeric *sentence,
  ...
  // Lexical features.
  fkey = encoder_.CreateFKey_W(EntityFeatureTemplateBigram::W, flags, WID);
  AddFeature(fkey, features);
  fkey = encoder_.CreateFKey_W(EntityFeatureTemplateBigram::pW, flags, pWID);
  AddFeature(fkey, features);
  fkey = encoder_.CreateFKey_W(EntityFeatureTemplateBigram::nW, flags, nWID);
  AddFeature(fkey, features);
  fkey = encoder_.CreateFKey_W(EntityFeatureTemplateBigram::ppW, flags, ppWID);
  AddFeature(fkey, features);
  fkey = encoder_.CreateFKey_W(EntityFeatureTemplateBigram::nnW, flags, nnWID);
  AddFeature(fkey, features);

  // POS features.
  fkey = encoder_.CreateFKey_P(EntityFeatureTemplateBigram::P, flags, PID);
  AddFeature(fkey, features);
  fkey = encoder_.CreateFKey_PP(EntityFeatureTemplateBigram::PpP, flags, PID, pPID);
  AddFeature(fkey, features);
  fkey = encoder_.CreateFKey_PP(EntityFeatureTemplateBigram::PnP, flags, PID, nPID);
  AddFeature(fkey, features);
  fkey = encoder_.CreateFKey_PPP(EntityFeatureTemplateBigram::PpPppP, flags, PID, pPID, ppPID);
  AddFeature(fkey, features);
  fkey = encoder_.CreateFKey_PPP(EntityFeatureTemplateBigram::PnPnnP, flags, PID, nPID, nnPID);
  AddFeature(fkey, features);
}
