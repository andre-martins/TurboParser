...
#include "MorphFeatureTemplates.h"
...
void MorphFeatures::AddBigramFeatures(SequenceInstanceNumeric *sentence,
  ...
  // POS tags.
  uint8_t PID = (position < sentence_length) ? (*cpostag_ids)[position] : TOKEN_STOP; // Current POS.
  // POS on the left.
  uint8_t pPID = (position > 0) ? (*cpostag_ids)[position - 1] : TOKEN_START;
  // POS on the right.
  uint8_t nPID = (position < sentence_length - 1) ? (*cpostag_ids)[position + 1] : TOKEN_STOP;
  ...
  // Lexical features.
  fkey = encoder_.CreateFKey_W(MorphFeatureTemplateBigram::W, flags, WID);
  AddFeature(fkey, features);
  fkey = encoder_.CreateFKey_W(MorphFeatureTemplateBigram::pW, flags, pWID);
  AddFeature(fkey, features);
  fkey = encoder_.CreateFKey_W(MorphFeatureTemplateBigram::nW, flags, nWID);
  AddFeature(fkey, features);
  fkey = encoder_.CreateFKey_W(MorphFeatureTemplateBigram::ppW, flags, ppWID);
  AddFeature(fkey, features);
  fkey = encoder_.CreateFKey_W(MorphFeatureTemplateBigram::nnW, flags, nnWID);
  AddFeature(fkey, features);

  // POS features.
  fkey = encoder_.CreateFKey_P(MorphFeatureTemplateBigram::P, flags, PID);
  AddFeature(fkey, features);
  fkey = encoder_.CreateFKey_P(MorphFeatureTemplateBigram::pP, flags, pPID);
  AddFeature(fkey, features);
  fkey = encoder_.CreateFKey_P(MorphFeatureTemplateBigram::nP, flags, nPID);
  AddFeature(fkey, features);
  fkey = encoder_.CreateFKey_P(MorphFeatureTemplateBigram::ppP, flags, ppPID);
  AddFeature(fkey, features);
  fkey = encoder_.CreateFKey_P(MorphFeatureTemplateBigram::nnP, flags, nnPID);
  AddFeature(fkey, features);
  fkey = encoder_.CreateFKey_PP(MorphFeatureTemplateBigram::PpP, flags, PID, pPID);
  AddFeature(fkey, features);
  // ** removing to test performance (start) **
  //fkey = encoder_.CreateFKey_PP(MorphFeatureTemplateBigram::PnP, flags, PID, nPID);
  //AddFeature(fkey, features);
  // ** removing to test performance (end) **
  fkey = encoder_.CreateFKey_PPP(MorphFeatureTemplateBigram::PpPppP, flags, PID, pPID, ppPID);
  AddFeature(fkey, features);
  // ** removing to test performance (start) **
  //fkey = encoder_.CreateFKey_PPP(MorphFeatureTemplateBigram::PnPnnP, flags, PID, nPID, nnPID);
  //AddFeature(fkey, features);
  //fkey = encoder_.CreateFKey_PPP(MorphFeatureTemplateBigram::PpPnP, flags, PID, pPID, nPID);
  //AddFeature(fkey, features);
  // ** removing to test performance (end) **
  ...
  //for (int l = 0; l < AID.size(); ++l) {
  //  uint8_t flag_prefix_length = l;
  //  fkey = encoder_.CreateFKey_WP(MorphFeatureTemplateBigram::A, flags, AID[l], flag_prefix_length);
  //  AddFeature(fkey, features);
  //}
  //for (int l = 0; l < pAID.size(); ++l) {
  //  uint8_t flag_prefix_length = l;
  //  fkey = encoder_.CreateFKey_WP(MorphFeatureTemplateBigram::pA, flags, pAID[l], flag_prefix_length);
  //  AddFeature(fkey, features);
  //}
  //for (int l = 0; l < nAID.size(); ++l) {
  //  uint8_t flag_prefix_length = l;
  //  fkey = encoder_.CreateFKey_WP(MorphFeatureTemplateBigram::nA, flags, nAID[l], flag_prefix_length);
  //  AddFeature(fkey, features);
  //}
  //for (int l = 0; l < ppAID.size(); ++l) {
  //  uint8_t flag_prefix_length = l;
  //  fkey = encoder_.CreateFKey_WP(MorphFeatureTemplateBigram::ppA, flags, ppAID[l], flag_prefix_length);
  //  AddFeature(fkey, features);
  //}
  //for (int l = 0; l < nnAID.size(); ++l) {
  //  uint8_t flag_prefix_length = l;
  //  fkey = encoder_.CreateFKey_WP(MorphFeatureTemplateBigram::nnA, flags, nnAID[l], flag_prefix_length);
  //  AddFeature(fkey, features);
  //}
  // ** removing to test performance (end) **
  ...
  for (int l = 0; l < ZID.size(); ++l) {
    uint8_t flag_suffix_length = l;
    fkey = encoder_.CreateFKey_WP(MorphFeatureTemplateBigram::Z, flags, ZID[l], flag_suffix_length);
    AddFeature(fkey, features);
  }
  for (int l = 0; l < pZID.size(); ++l) {
    uint8_t flag_suffix_length = l;
    fkey = encoder_.CreateFKey_WP(MorphFeatureTemplateBigram::pZ, flags, pZID[l], flag_suffix_length);
    AddFeature(fkey, features);
  }
  for (int l = 0; l < nZID.size(); ++l) {
    uint8_t flag_suffix_length = l;
    fkey = encoder_.CreateFKey_WP(MorphFeatureTemplateBigram::nZ, flags, nZID[l], flag_suffix_length);
    AddFeature(fkey, features);
  }
  for (int l = 0; l < ppZID.size(); ++l) {
    uint8_t flag_suffix_length = l;
    fkey = encoder_.CreateFKey_WP(MorphFeatureTemplateBigram::ppZ, flags, ppZID[l], flag_suffix_length);
    AddFeature(fkey, features);
  }
  for (int l = 0; l < nnZID.size(); ++l) {
    uint8_t flag_suffix_length = l;
    fkey = encoder_.CreateFKey_WP(MorphFeatureTemplateBigram::nnZ, flags, nnZID[l], flag_suffix_length);
    AddFeature(fkey, features);
  }
