...
void SemanticFeatures::AddArcFeatures(SemanticInstanceNumeric* sentence,
    ...
    //Corentin Ribeyre
    bool use_morphological_features = true;
    ...
    // Codewords for accommodating morphological features information (Corentin Ribeyre).
    uint16_t HFID, MFID; //Corentin Ribeyre
    ...
    if(use_morphological_features && use_dependency_features){
        for (int j = 0; j < sentence->GetNumMorphFeatures(predicate); ++j) {
            HFID = sentence->GetMorphFeature(predicate, j);
            CHECK_LT(HFID, 0xffff);
            MFID = sentence->GetMorphFeature(argument, j); //Strong assumptions --> same morphological features
            ...
            if (j >= 0xf) {
                ...
                MFID = (MFID << 4) | ((uint16_t) j);
            }

            fkey = encoder_.CreateFKey_W(SemanticFeatureTemplateArc::HF, flags, HFID);
            AddFeature(fkey, features);
            fkey = encoder_.CreateFKey_WW(SemanticFeatureTemplateArc::HWF, flags, HWID, HFID);
            AddFeature(fkey, features);
            fkey = encoder_.CreateFKey_WW(SemanticFeatureTemplateArc::HPF, flags, HPID, HFID);
            AddFeature(fkey, features);

            fkey = encoder_.CreateFKey_W(SemanticFeatureTemplateArc::MF, flags, MFID);
            AddFeature(fkey, features);
            fkey = encoder_.CreateFKey_WW(SemanticFeatureTemplateArc::MWF, flags, MWID, MFID);
            AddFeature(fkey, features);
            fkey = encoder_.CreateFKey_WW(SemanticFeatureTemplateArc::MPF, flags, MPID, MFID);
            AddFeature(fkey, features);
