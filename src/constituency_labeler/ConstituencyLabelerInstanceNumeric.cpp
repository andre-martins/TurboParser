...
void ConstituencyLabelerInstanceNumeric::Initialize(
  ...
  for (int i = 0; i < instance->GetNumConstituents(); ++i) {
    const std::string &label = instance->GetConstituentLabel(i);
    int id = dictionary.GetLabelId(label);
    CHECK_LT(id, 0xffff);
    if (id < 0) id = TOKEN_UNKNOWN;
    constituent_labels_[i] = id;
  }
}
